    ...
    <div style={{display:'flex',gap:24}}>
      <div style={{flex:1,minWidth:0}}>
        ...
        <Cd style={{overflow:'hidden',marginBottom:24}}>
          <div style={{padding:'16px 20px',borderBottom:`1px solid ${C.bd}`,display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontSize:16,color:C.tx}}>Active Onboardings</span><Btn onClick={()=>nav('ALL_ONBOARDINGS')}>View All →</Btn></div>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}><thead><tr style={{borderBottom:`1px solid ${C.bd}`}}>{['Client','Phase','Progress','Days','Status',''].map(h=><TH key={h}>{h}</TH>)}</tr></thead>
            <tbody>{OBS.filter(o=>o.status!=='STOPPED').slice(0,5).map(o=><tr key={o.id} style={{borderBottom:`1px solid ${C.bd}`,cursor:'pointer'}} onClick={()=>{setSelOB(o.id);if(o.status==='COMPLETE')nav('ONBOARDING_COMPLETE',o.id)}}>
              ...
              <td style={{padding:'12px 16px',width:140}}><div style={{display:'flex',alignItems:'center',gap:8}}><PBar pct={o.progress}/><span style={{fontSize:11,color:C.t2,fontFamily:"'Source Code Pro','Fira Code',monospace",minWidth:32}}>{o.progress}%</span></div></td>
        ...
        <Cd style={{padding:20}}>
          <div style={{fontSize:14,color:C.tx,marginBottom:16}}>Phase Progress — {ob?.clientName}</div>
          <div style={{display:'flex',gap:4}}>{PHASES.map(p=>{const pp=ob?(p.num<ob.phase?100:p.num===ob.phase?Math.min(90,((ob.currentTask-p.tasks[0])/p.tasks.length)*100):0):0;const ia=p.num===ob?.phase;
            return <div key={p.num} style={{flex:1}}><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}><div style={{width:28,height:28,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:600,fontFamily:"'Source Code Pro','Fira Code',monospace",background:pp===100?C.ok:ia?C.ac:C.bd,color:pp===100||ia?'#fff':C.t2}}>{pp===100?'✓':p.num}</div><span style={{fontSize:11,color:ia?C.tx:C.t2,fontWeight:ia?600:400}}>{p.name}</span></div><PBar pct={pp} h={4} color={pp===100?C.ok:ia?C.ac:C.bd}/></div>})}</div>
        </Cd>
      </div>
      <div style={{width:320,flexShrink:0}}><Cd style={{padding:20,position:'sticky',top:0}}>
        <div style={{fontSize:16,color:C.tx,marginBottom:16}}>My Tasks</div>
