...
export function AllOnboardings({ nav, obFilter, setObFilter }) {
  ...
  return <div><Hdr title="All Onboardings" sub="Segregated Funds — complete registry"/>
    <div style={{display:'flex',gap:8,marginBottom:20}}>{['ALL','ACTIVE','COMPLETE','STOPPED'].map(x=><button key={x} onClick={()=>sF(x)} style={{padding:'8px 16px',borderRadius:6,border:`1px solid ${f===x?C.ac:C.bd}`,background:f===x?C.ad:'transparent',color:f===x?C.ac:C.t2,fontSize:12,cursor:'pointer',fontWeight:f===x?600:400}}>{x==='ALL'?'All':x==='ACTIVE'?'Active':x==='COMPLETE'?'Complete':'Stopped'}</button>)}</div>
    <Cd style={{overflow:'hidden'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}><thead><tr style={{borderBottom:`1px solid ${C.bd}`}}>{['ID','Client','AUM','Manager','Phase','Progress','Days','Risk','Status',''].map(h=><TH key={h}>{h}</TH>)}</tr></thead>
      <tbody>{fl.map(o=><tr key={o.id} style={{borderBottom:`1px solid ${C.bd}`}}>
        ...
        <td style={{padding:'12px 16px'}}><div style={{display:'flex',alignItems:'center',gap:6}}><PBar pct={o.progress}/><span style={{fontSize:11,color:C.t2,fontFamily:"'Source Code Pro','Fira Code',monospace"}}>{o.progress}%</span></div></td>
